  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `http://localhost:5000/upload?t=${Date.now()}`,
        fd,
      ...
      setResults({
        plots: data.plots.map(u => `http://localhost:5000${u}?t=${timestamp}`),
      ...
      if (autoDownloadPlots && data.plots && data.plots.length > 0) {
        ...
        setTimeout(() => {
          window.location.href = `http://localhost:5000/download-plots/${data.session_id}`;
        }, 1000);
      }
