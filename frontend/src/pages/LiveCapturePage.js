...
export default function LiveCapturePage() {
  ...
  useEffect(() => {
    ...
    if (isCapturing) {
      ...
      countPollingRef.current = setInterval(() => {
        axios.get('http://localhost:5000/live-data')
          .then(({data}) => {
  ...
  const fetchLiveData = async () => {
    ...
    try {
      const { data } = await axios.get('http://localhost:5000/live-data');
  ...
  const startCapture = async () => {
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:5000/start-capture');
  ...
  const processData = async (sessionId) => {
    ...
    try {
      ...
      const { data } = await axios.get(`http://localhost:5000/process-capture/${sessionId}`);
      ...
      if (data.cleaned_df && data.cleaned_df.length > 0) {
        ...
        const formattedPlots = (data.plots || []).map(u => 
          u.startsWith('http') ? `${u}?t=${Date.now()}` : `http://localhost:5000${u}?t=${Date.now()}`
        );
        ...
        if (autoDownloadPlots && formattedPlots.length > 0) {
          ...
          setTimeout(() => {
            window.location.href = `http://localhost:5000/download-plots/${sessionId}`;
          }, 1000);
        }
  ...
  const stopCapture = async () => {
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:5000/stop-capture');
  ...
  const downloadCaptureData = () => {
    if (!sessionId) return;
    window.location.href = `http://localhost:5000/download-data/${sessionId}`;
  };
